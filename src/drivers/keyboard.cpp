    ...
    namespace drivers
    {
        ...
        void KeyboardDriver::Activate()
        {
            while(commandport.Read() & 0x1)
                dataport.Read(); // this ensures we can capture and discard keystrikes from when the OS is booting.

            commandport.Write(0xAE); // tells pic to start sending keyboard inputs.
